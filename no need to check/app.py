...
def new_garena_scraper():
    ...
    if os.getenv('WEBSHARE_ENABLED', 'false').lower() == 'true' or os.getenv('GARENA_PROXY_ENABLED', 'false').lower() == 'true':
...
def garena_payment_init(player_id: str, session_data=None):
    if session_data is None:
        ...
        # Try across up to 3 random Rotating Residential IPs
        for attempt in range(3):
            scraper = new_garena_scraper()

            # Step 1: Initial GET to fetch fresh mspid2 session cookie
            init_headers = {
            ...
            if datadome:
                cookie_parts.append(f"datadome={datadome}")
